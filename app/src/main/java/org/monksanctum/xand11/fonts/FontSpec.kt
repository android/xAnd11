...
class FontSpec {
    ...
    override fun toString(): String {
        if ((NONE == mSpecs[PIXEL_SIZE]) xor (NONE == mSpecs[POINT_SIZE])) {
            ...
            if (NONE == mSpecs[PIXEL_SIZE]) {
                mSpecs[PIXEL_SIZE] = Math.round(Integer.parseInt(mSpecs[POINT_SIZE]) * sDpi / 722.7).toString()
            } else {
                mSpecs[POINT_SIZE] = Math.round(Integer.parseInt(mSpecs[PIXEL_SIZE]) * 722.7 / sDpi).toString()
            }
        }
    ...
    companion object {
                    ...
                    for (weight in WEIGHTS) {
                        ...
                        for (slant in SLANTS) {
                            ...
                            for (registry in REGISTRIES) {
                                ...
                                fontSpecs.add(FontSpec(TextUtils.join("-", specs)))
                            }
                        }
                    }
                }
        ...
        fun initDpi(context: Context) {
            sDpi = context.resources.displayMetrics.densityDpi
        }
    }
}
