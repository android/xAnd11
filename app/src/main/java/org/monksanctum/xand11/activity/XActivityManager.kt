...
class XActivityManager(private val mService: XService) {
    ...
    fun onWindowMapped(window: XWindow) {
        val intent = Intent(mService, XActivity::class.java)
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_NEW_DOCUMENT
                or Intent.FLAG_ACTIVITY_MULTIPLE_TASK)
        intent.putExtra(XActivity.EXTRA_WINDOW_ID, window.id)
        mService.startActivity(intent)
    }

    fun onWindowUnmapped(window: XWindow) {
