...
class GraphicsProtocol(private val mManager: GraphicsManager, private val mFontManager: FontManager) : Dispatcher.PacketHandler {
    ...
    override fun handleRequest(client: Client, reader: PacketReader, writer: PacketWriter) {
        when (reader.majorOpCode) {
            Request.CREATE_GC -> handleCreateGc(reader)
            Request.CHANGE_GC -> handleChangeGc(reader)
            Request.FREE_GC -> handleFreeGc(reader)
            Request.CREATE_PIXMAP -> handleCreatePixmap(reader)
            Request.FREE_PIXMAP -> handleFreePixmap(reader)
            Request.PUT_IMAGE -> handlePutImage(reader)
            Request.GET_IMAGE -> handleGetImage(reader, writer)
            Request.GET_GEOMETRY -> handleGetGeometry(reader, writer)
            Request.QUERY_BEST_SIZE -> handleRequestBestSize(reader, writer)
            Request.COPY_PLANE -> handleCopyPlane(reader, writer)
        }
    }
    ...
    private fun handlePutImage(reader: PacketReader) {
        ...
        synchronized(drawable) {
            drawable.lockCanvas(context).drawBitmap(bitmap, x.toFloat(), y.toFloat(), context.paint)
            if (DEBUG)
                Log.d(TAG, "Drawing bitmap " + x + " " + y + " " + bitmap.width + " "
                        + bitmap.height)
    ...
    private fun handleCopyPlane(reader: PacketReader, writer: PacketWriter) {
        ...
        synchronized(dest) {
            val c = dest.lockCanvas(gc)
            c.drawBitmap(bitmap, destX.toFloat(), destY.toFloat(), gc.paint)
            dest.unlockCanvas()
        }
    }
    ...
    companion object {
        ...
        private val HANDLED_OPS = byteArrayOf(Request.CREATE_GC, Request.CHANGE_GC, Request.FREE_GC, Request.CREATE_PIXMAP, Request.FREE_PIXMAP, Request.PUT_IMAGE, Request.GET_IMAGE, Request.GET_GEOMETRY, Request.QUERY_BEST_SIZE, Request.COPY_PLANE)
