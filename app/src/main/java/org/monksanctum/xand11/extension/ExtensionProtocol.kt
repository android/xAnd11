...
class ExtensionProtocol(private val mExtensionManager: ExtensionManager) : Dispatcher.PacketHandler {
    ...
    override fun handleRequest(client: Client, reader: PacketReader, writer: PacketWriter) {
        when (reader.majorOpCode) {
            Request.QUERY_EXTENSION -> handleQueryExtension(reader, writer)
            Request.LIST_EXTENSIONS -> handleListExtension(reader, writer)
        }
    }
    ...
    companion object {
        ...
        private val HANDLED_OPS = byteArrayOf(Request.QUERY_EXTENSION, Request.LIST_EXTENSIONS)
    }
}
