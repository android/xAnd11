...
public class AtomManager {
    ...
    public static final int ATOM_NONE = 0;
    public static final int WM_NAME = 39;

    // Global because they are so common.
    private static AtomManager sInstance;

    private final ArrayList<String> mAtoms = new ArrayList<>();

    public AtomManager() {
        addPredifinedAtoms();
    }

    private void addPredifinedAtoms() {
        mAtoms.add(0, null);
    ...
    public int internAtom(String str) {
        for (int i = 0; i < mAtoms.size(); i++) {
            if (str.equals(mAtoms.get(i))) {
                return i;
            }
        }
        int index = mAtoms.size();
        mAtoms.add(str);
        return index;
    }

    public String getString(int id) throws AtomError {
        if (id >= mAtoms.size()) {
            throw new AtomError(id);
        }
        String value = mAtoms.get(id);
        if (value == null) {
            throw new AtomError(id);
        }
        return value;
