...
class AtomProtocol : Dispatcher.PacketHandler {
    ...
    override fun handleRequest(client: Client, reader: PacketReader, writer: PacketWriter) {
        when (reader.majorOpCode) {
            Request.INTERN_ATOM -> handleInternAtom(reader, writer)
            Request.GET_ATOM_NAME -> handleGetAtomName(reader, writer)
        }
    }
    ...
    companion object {
        ...
        private val HANDLED_OPS = byteArrayOf(Request.INTERN_ATOM, Request.GET_ATOM_NAME)
    }
}
